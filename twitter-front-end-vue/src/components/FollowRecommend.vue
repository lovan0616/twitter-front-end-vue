<template>
  <div class="card">
      ...
      >
        ...
        >
          ...
          <div class="follow-badge ml-auto">
            <div
              class="following badge badge-pill badge-primary"
              v-if="recommend.isFollowed"
            >
              正在跟隨
            </div>
            <div
              class="deleteFollow badge badge-pill"
              v-if="recommend.isFollowed"
              @click.stop.prevent="unfollow(recommend)"
            >
              取消跟隨
            </div>
            <div
              class="unfollow badge badge-pill badge-outline-primary"
              v-else
              @click.stop.prevent="follow(recommend)"
            >
              跟隨
            </div>
          </div>
    ...
    <div
      class="card-footer more"
      @click.stop.prevent="showAllRecommends"
      v-if="!recommendsAllShowed"
    >
      顯示更多
    </div>
  </div>
  ...
  methods: {
    async fetchData(amount) {
      try {
        ...
        const { data } = response;
        this.recommends = [...data].filter(
          (recommend, index, self) => self.indexOf(recommend) < amount
        );
      } catch (error) {
        console.log(error);
        Toast.fire({
